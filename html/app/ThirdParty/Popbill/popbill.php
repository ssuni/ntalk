<?php namespace Popbill\PopbillBase;
...
use Popbill\Linkhub\Linkhub as Linkhub;
use Exception;
class PopbillBase
{
    ...
    private $__requestMode;

    public function __construct($LinkID, $SecretKey)
    {
        $this->Linkhub = Linkhub::getInstance('SELFNICK', $SecretKey);
        $this->scopes[] = 'member';
        $this->__requestMode = 'CURL';
        $this->Token_Table = array();
    }
    ...
    private function getsession_Token($CorpNum)
    {
        ...
        if (array_key_exists($CorpNum, Token_Table)) {
        ...
        if ($Refresh) {
            try {
                $targetToken = Linkhub::getToken($this->IsTest ? PopbillBase::ServiceID_TEST : PopbillBase::ServiceID_REAL, $CorpNum,$this->scopes);

            } catch (LinkhubException $le) {
                throw new PopbillException($le->getMessage(), $le->getCode());
            }
            $Token_Table[$CorpNum] = $targetToken;
        }
    ...
    protected function executeCURL($uri, $CorpNum = null, $userID = null, $isPost = false, $action = null, $postdata = null, $isMultiPart = false, $contentsType = null)
    {
        if (__requestMode != "STREAM") {
            ...
            if (is_null($CorpNum) == false) {
                $header[] = 'Authorization: Bearer ' . self::getsession_Token($CorpNum);
            }
...
//            var_dump((IsTest ? PopbillBase::ServiceURL_TEST : PopbillBase::ServiceURL_REAL) . $uri);
//            exit;
