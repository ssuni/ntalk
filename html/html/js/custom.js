    ...
    _Join = (function(){
        return {
            ...
            locationHTML: function(locationData){
                ...
                for(i=0; i < locationData_langth; i++){
            ...
            locationSecond: function(locationData){
                ...
                for(i=0; i < locationData[select_index].childs.length; i++){
                    var html = '';
            ...
            location_json: function(){
                $.ajax({
                    ...
                    REGISTER.find('#first').change(function(){ //1차 지역 선택시
                        var second_select = REGISTER.find('#second');
    ...
    (function(){
        ...
        REGISTER.on('click', '#s_essentialContainer .s_idContainer label input[type=button]', function(){ //아이디 중복확인
            var id_val = REGISTER.find('#s_essentialContainer .s_idContainer label input').val(),
                ...
                confirm_val = {id:id_val, division:division_val},
                id_name = "아이디";
            ...
            var nick_val = REGISTER.find('.s_nickContainer label input[type=text]').val(),
                ...
                confirm_val = {id:nick_val, division:division_val},
                nick_name = "닉네임";
        ...
        second_request.on('click', '.re_number label input', function(){ //전화인증 번호 재전송
            ...
            var request_number = second_request.find('#certification').val(),
                ...
                request_value = {phone:phone_number, certification:request_number},
            ...
            list_html: function(dataContent, currentPage, nextPage, lastPage, dataList, _this_text){
                ...
                for(var variable in object) {
                    if(object.hasOwnProperty(variable)) {
                        ...
                        html += '<div class="userId" title="아이디">'+userId+'</div>';
                ...
                TIMELIST.find('.timeLineContent').html(html);
                /*if(_this_text == ''){
                    _Time.page_html(lastPage); // 페이지 네이션 추가
                    _Time.addTimeLine(); // 타임라인 작성 창 추가
                }*/
            },
            /*page_html: function(lastPage) {
                var html = '';
                ...
                $('#pagination').twbsPagination({
                    ...
                    onPageClick: function(event,page){
                        ...
                        var _this = $(this),
                            _this_text = TIMELIST.find('.pagination .page-item.active a').text(),
                            active_index = TIMELIST.find('.pagination .page-item.active').index(),
                            select_text = '?page='+_this_text+'';
console.log(select_text);
                            TIMELIST.find('.timeLineContent li').remove();
                            _Time.timeLineList(select_text);
                    },
            ...
            addTimeLine: function() {
                ...
                html += '<div id="menuTap">';
                html += '<div class="writeTimeLine">';
                html += '<form action="/api/Timeline/timeline_insert" method="POST" id="writeTime">';
                html += '<textarea class="noresize" name="comment" form="writeTime" cols="50" autofocus required wrap="hard" placeholder="타임라인을 작성하세요."></textarea>';
                html += '<input type="submit" value="타임라인 등록">';
                html += '</form>';
                html += '</div>';
                html += '</div>';
...
console.log(time);*/
        (function(){ // 타임라인 자동스크롤 이벤트
            ...
            /*DOCUMENT.on('click', '#timeLineContainer nav #pagination li', function(){
                var _this = $(this),
                    _this_text = _this.find('a').text(),
                    active_index = TIMELIST.find('.pagination .page-item.active').index();

                if(_this_text != '<<' && _this_text != '>>'){ // 페이지 번호 클릭 시
                    var select_text = '?page='+_this_text+'';

                    TIMELIST.find('.timeLineContent li').remove();
                    _Time.timeLineList(select_text);
                    _this.addClass('active');
                    _this.siblings().removeClass('active');
                }else if(_this_text == '<<'){ // 이전 버튼 클릭시
                    var active_index = TIMELIST.find('.pagination .page-item.active').index();

                    if(active_index != "1"){
                        var select_text = '?page='+(active_index-1)+'';

                        TIMELIST.find('.timeLineContent li').remove();
                        _Time.timeLineList(select_text);
                        TIMELIST.find('.pagination .page-item').eq(active_index-2).addClass('active');
                        TIMELIST.find('.pagination .page-item').eq(active_index-2).siblings().removeClass('active');
                    }
                }else if(_this_text == ">>"){ // 다음 버튼 클릭 시
                    var active_index = TIMELIST.find('.pagination .page-item.active').index(),
                        page_length = TIMELIST.find('.pagination .page-item').length,
                        add_active = active_index;

                    if(active_index < page_length){
                        var select_text = '?page='+(active_index+1)+'';

                        TIMELIST.find('.timeLineContent li').remove();
                        _Time.timeLineList(select_text);
                        TIMELIST.find('.pagination .page-item').eq(active_index).addClass('active');
                        TIMELIST.find('.pagination .page-item').eq(active_index).siblings().removeClass('active');
                    }
                }
            }); */
        //    $(win.document.body).on()
            TIMELIST.find('.timeLineContent').on('click', '.userModify', function(){ //타임라인 수정버튼 클릭 이벤트
                ...
                var _this = $(this);
                    ...
                    request_value = {t_idx:number,comment:content},
                ...
                var _this = $(this),
                    ...
                    request_value = {content_id:number};
                ...
                if(popupProfile_length == 0 && popupReport_length == 0){
                    ...
                    popupProfile.css({'top': position_top, 'left': position_left});
                ...
                if(popupReport_length == 0 && popupProfile_length == 0){
                    _Time.report_html();
                    var popupReport = $('#popupReport');
                    popupReport.css({'top': position_top, 'left': position_left});
            ...
            DOCUMENT.on('click', '#popupProfile .profileInfo .profileTitle div', function(){ //프로필 팝업 닫기 번큰 이벤트
            ...
            }).on('click', '#popupProfile div .profileEvent div a', function() {
                ...
                var _this = $(this),
                    ...
                    request_value = {comment:textarea_val},
